...
def get_app_bundle():
    """
    For a PyInstaller .app, sys.executable is:
      /path/to/M2 Tracker.app/Contents/MacOS/M2 Tracker
    Walk up to find the .app root so we can register the bundle, not the raw binary.
    """
    import sys
    ...
    for _ in range(6):
        ...
        if parent == path:
            break
        path = parent
    # Fallback when running outside a bundle (e.g. during dev)
    import sys
...
def set_launch_at_login(enabled):
    bundle_path = get_app_bundle()

    # ── Try SMAppService first (macOS 13 Ventura+) ─────────────────────────────
    # Proper modern API — shows up in System Settings → General → Login Items.
    # No plist needed, no duplicate launches.
    try:
        ...
        return  # handled — don't fall through to LaunchAgent
    except Exception:
        pass  # ServiceManagement not available — fall through

    # ── Fallback: LaunchAgent plist (macOS 12 and below) ───────────────────────
    # Use "open -a <bundle>" so macOS handles the launch properly and won't
    # spawn a raw Python/binary process that causes a second instance.
    if enabled:
        plist = {
            "Label": BUNDLE_ID,
            "ProgramArguments": ["open", "-a", bundle_path],
            "RunAtLoad": True,
            "KeepAlive": False,
        }
